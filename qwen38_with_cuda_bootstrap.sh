...
#
# Quick start:
#   chmod +x ./qwen38_llamacpp_claude_v2_cuda-bootstrap.sh
#   CUDA_VISIBLE_DEVICES=0 ./qwen38_llamacpp_claude_v2_cuda-bootstrap.sh install
#   ./qwen38_llamacpp_claude_v2_cuda-bootstrap.sh claude
#
# Second physical GPU (independent server/model):
#   CUDA_VISIBLE_DEVICES=1 INSTANCE=agent2 PORT=8081 ./qwen38_llamacpp_claude_v2_cuda-bootstrap.sh install
#   INSTANCE=agent2 PORT=8081 ./qwen38_llamacpp_claude_v2_cuda-bootstrap.sh claude
#
# Same GPU, second independent Claude Code agent: share the same model server (no second
# copy of 27B weights/KV in VRAM): launch ./qwen38_llamacpp_claude_v2_cuda-bootstrap.sh claude in another shell.
#
# Useful overrides:
...
UV_CACHE_DIR_LOCAL="$STATE_DIR/uv-cache"
CUDA_BOOTSTRAP_ENV="$STATE_DIR/cuda-bootstrap-py"
CUDA_PRIVATE_ROOT="$STATE_DIR/cuda-12.9.1"
CUDA_CHANNEL="${CUDA_CHANNEL:-https://conda.anaconda.org/nvidia/label/cuda-12.9.1}"
PY_RATTLER_VERSION="${PY_RATTLER_VERSION:-0.25.0}"
CUDA_BOOTSTRAP_VERSION="${CUDA_BOOTSTRAP_VERSION:-12.9.1}"
CUBLAS_BOOTSTRAP_VERSION="${CUBLAS_BOOTSTRAP_VERSION:-12.9.1.4}"
...
mkdir -p "$STATE_DIR" "$BUILD_ROOT" "$MODEL_DIR" "$LOG_DIR" "$RUN_DIR" "$BENCH_DIR" "$HF_HOME" "$TMP_ROOT" "$CLAUDE_ROOT" "$TEMPLATE_DIR" "$TOOLS_DIR/bin" "$UV_CACHE_DIR_LOCAL" "$CLAUDE_CONFIG_DIR_LOCAL" "$CLAUDE_TMP_DIR_LOCAL" || {
...
install_basic_deps() {
  local missing=()
  local c
  for c in git cmake curl jq gcc g++ make python3; do
    need "$c" || missing+=("$c")
  done
  ...
  if need apt-get; then
    ...
    $SUDO apt-get install -y build-essential cmake ninja-build git curl jq ca-certificates python3 || die "apt dependency install failed"
  elif need dnf; then
    $SUDO dnf install -y gcc gcc-c++ cmake ninja-build make git curl jq ca-certificates python3 || die "dnf dependency install failed"
  elif need pacman; then
    $SUDO pacman -Sy --needed --noconfirm base-devel cmake ninja git curl jq ca-certificates python || die "pacman dependency install failed"
  else
...
find_nvcc() {
  # Print a usable compiler path or return 1. Never provision from this function:
  # `plan` must remain non-destructive.
  local want_cuda12=0
  [ "$GPU_ARCH" -lt 75 ] && want_cuda12=1

  local candidates=()
  [ -n "${CUDA_HOME:-}" ] && candidates+=("$CUDA_HOME/bin/nvcc")
  [ -n "${CUDACXX:-}" ] && candidates+=("$CUDACXX")
  candidates+=(
    "$CUDA_PRIVATE_ROOT/bin/nvcc"
  ...
  local p release major
  for p in "${candidates[@]}"; do
    [ -x "$p" ] || continue
    release="$("$p" --version 2>/dev/null | sed -n 's/.*release \([0-9][0-9]*\)\..*/\1/p' | tail -n1)"
    major="${release:-0}"
    if [ "$want_cuda12" -eq 1 ] && [ "$major" -ge 13 ]; then
      continue
    fi
    printf '%s\n' "$p"
    return 0
...
ensure_local_uv() {
  # Use an existing uv if present; otherwise install a private unmanaged uv under STATE_DIR.
  # All uv caches and Python bootstrap state are also redirected under STATE_DIR.
  local uv_bin=""
  if need uv; then
    uv_bin="$(command -v uv)"
  elif [ -x "$TOOLS_DIR/bin/uv" ]; then
    uv_bin="$TOOLS_DIR/bin/uv"
  else
    say "uv is missing; installing a private unmanaged uv beside this script" >&2
    local installer="$TMP_ROOT/uv-install.sh"
    curl -LsSf https://astral.sh/uv/install.sh -o "$installer" || die "failed to download the official uv installer"
    env UV_UNMANAGED_INSTALL="$TOOLS_DIR/bin" UV_NO_MODIFY_PATH=1 sh "$installer" 1>&2 \
      || die "private uv installation failed"
    uv_bin="$TOOLS_DIR/bin/uv"
  fi
  [ -x "$uv_bin" ] || die "uv bootstrap returned no executable: $uv_bin"
...
bootstrap_private_cuda() {
  # Conda-free frontend: uv hosts Python/py-rattler; py-rattler directly solves and
  # installs NVIDIA's official conda packages into a plain private prefix. There is
  # NO conda/mamba executable/environment, NO NVIDIA .run installer, NO driver change,
  # NO sudo, and NO /usr/local mutation.
  if [ -x "$CUDA_PRIVATE_ROOT/bin/nvcc" ]; then
    printf '%s\n' "$CUDA_PRIVATE_ROOT/bin/nvcc"
    return 0
  fi
  ...
  say "nvcc is missing/incompatible; provisioning private CUDA $CUDA_BOOTSTRAP_VERSION" >&2
  info "bootstrap Python env : $CUDA_BOOTSTRAP_ENV" >&2
  info "CUDA prefix         : $CUDA_PRIVATE_ROOT" >&2
  info "NVIDIA channel      : $CUDA_CHANNEL" >&2
  info "frontend            : uv + Python/py-rattler (NO conda/mamba executable)" >&2

  mkdir -p "$CUDA_PRIVATE_ROOT" "$UV_CACHE_DIR_LOCAL" || die "cannot create private CUDA directories"

  if [ ! -x "$py" ]; then
    "$uv_bin" venv "$CUDA_BOOTSTRAP_ENV" --python "$(command -v python3)" --no-managed-python 1>&2 \
      || die "uv failed to create CUDA bootstrap Python environment"
  fi

  "$uv_bin" pip install --python "$py" "py-rattler==$PY_RATTLER_VERSION" 1>&2 \
    || die "failed to install py-rattler==$PY_RATTLER_VERSION"

  "$py" - "$CUDA_PRIVATE_ROOT" "$CUDA_CHANNEL" "$CUDA_BOOTSTRAP_VERSION" "$CUBLAS_BOOTSTRAP_VERSION" <<'PYCUDA' 1>&2 \
    || die "Python/Rattler CUDA bootstrap failed"
import asyncio
import os
import sys
...
specs = [
    f"cuda-minimal-build {cuda_version}",
    f"libcublas-dev {cublas_version}",
]

async def main():
    print("CUDA bootstrap: solving NVIDIA packages", flush=True)
    ...
    print(f"CUDA bootstrap: solved {len(records)} packages", flush=True)
    root.mkdir(parents=True, exist_ok=True)
    await install(records=records, target_prefix=str(root))
    print(f"CUDA bootstrap: installed into {root}", flush=True)

asyncio.run(main())

candidates = [
    root / "bin" / "nvcc",
    root / "targets" / "x86_64-linux" / "bin" / "nvcc",
]
candidates += list(root.glob("targets/*/bin/nvcc"))
nvcc = next((p for p in candidates if p.is_file() and os.access(p, os.X_OK)), None)
if nvcc is None:
    print("ERROR: CUDA packages installed but nvcc was not found.", file=sys.stderr)
    for p in candidates:
...
print(f"nvcc={stable}", flush=True)
PYCUDA

  [ -x "$CUDA_PRIVATE_ROOT/bin/nvcc" ] || die "private CUDA bootstrap completed but nvcc is unavailable"
  "$CUDA_PRIVATE_ROOT/bin/nvcc" --version 1>&2 || die "private nvcc cannot execute"
  printf '%s\n' "$CUDA_PRIVATE_ROOT/bin/nvcc"
}

ensure_nvcc() {
  local nv=""
  nv="$(find_nvcc 2>/dev/null || true)"
  if [ -z "$nv" ]; then
    nv="$(bootstrap_private_cuda)"
  fi
  ...
  if [ "$GPU_ARCH" -lt 75 ] && [ "$major" -ge 13 ]; then
    # A host CUDA 13 compiler cannot compile P40/V100. Force the private CUDA-12 path.
    nv="$(bootstrap_private_cuda)"
  fi
  [ -x "$nv" ] || die "no usable nvcc could be found or provisioned"
  printf '%s\n' "$nv"
}
...
configure_cuda_env() {
  local nv="$1"
  CUDA_ROOT="$(cd "$(dirname "$nv")/.." && pwd -P)"
  local target="$CUDA_ROOT/targets/x86_64-linux"
  CUDA_LIB_PATH="$CUDA_ROOT/lib64:$CUDA_ROOT/lib:$target/lib:$target/lib/stubs"
  CUDA_INC_PATH="$CUDA_ROOT/include:$target/include"

  export PATH="$(dirname "$nv"):$PATH"
...
build_llama() {
  ...
  CUDACXX="$NVCC" cmake -S "$SRC_DIR" -B "$BUILD_DIR" \
    ...
    -DCMAKE_BUILD_RPATH="$CUDA_LIB_PATH" \
  ...
  LOCAL_LD_PATH="$BUILD_DIR/bin:$BUILD_DIR/src:$BUILD_DIR/ggml/src:$BUILD_DIR/ggml/src/ggml-cuda:$CUDA_LIB_PATH"
...
load_existing_build() {
  ...
  local cuda_extra=""
  if [ -x "$CUDA_PRIVATE_ROOT/bin/nvcc" ]; then
    local cuda_target="$CUDA_PRIVATE_ROOT/targets/x86_64-linux"
    cuda_extra="$CUDA_PRIVATE_ROOT/lib64:$CUDA_PRIVATE_ROOT/lib:$cuda_target/lib:$cuda_target/lib/stubs"
  fi
  LOCAL_LD_PATH="$BUILD_DIR/bin:$BUILD_DIR/src:$BUILD_DIR/ggml/src:$BUILD_DIR/ggml/src/ggml-cuda${cuda_extra:+:$cuda_extra}"
  if [ -n "${LD_LIBRARY_PATH:-}" ]; then LOCAL_LD_PATH="$LOCAL_LD_PATH:$LD_LIBRARY_PATH"; fi
...
NVCC=$(printf '%q' "$NVCC")
CUDA_ROOT=$(printf '%q' "$CUDA_ROOT")
CUDA_LIB_PATH=$(printf '%q' "$CUDA_LIB_PATH")
HOST=$(printf '%q' "$HOST")
PORT=$(printf '%q' "$PORT")
...
plan() {
  ...
  nv="$(find_nvcc 2>/dev/null || true)"
  if [ -z "$nv" ]; then
    nv="AUTO: private CUDA $CUDA_BOOTSTRAP_VERSION will be provisioned under $CUDA_PRIVATE_ROOT"
  fi
  kv="$(choose_kv_type)"
  nonweight="$(estimated_nonweight_mib "$kv")"
  budget=$((GPU_FREE_MIB - nonweight))
...
Commands:
  install       deps + auto-local CUDA if needed + build + quant select/download + full-ctx fit proof + bench + deploy + smokes
...
CUDA bootstrap:
  If nvcc is absent (or CUDA 13 is unusable for P40/V100), install automatically provisions
  private CUDA 12.9.1 with uv + Python/py-rattler under .qwen38-llamacpp-claude/.
  No conda/mamba executable, no NVIDIA .run installer, no driver change, no /usr/local write.
