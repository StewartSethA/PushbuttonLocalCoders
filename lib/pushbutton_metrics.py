...
REMOTE_URL = os.environ.get("PUSHBUTTON_METRICS_URL","https://raw.githubusercontent.com/StewartSethA/PushbuttonLocalCoders/main/benchmarks/aggregate.json")
...
def telemetry_config() -> dict:return _read_json(CONFIG/"telemetry.json", {})

def set_telemetry(enabled: bool, upload_url: str | None = None) -> pathlib.Path:
    CONFIG.mkdir(parents=True, exist_ok=True); obj={"enabled":bool(enabled)}
...
def queue_report(report: dict) -> pathlib.Path | None:
    ...
    compact={"schema_version":1,"timestamp_utc":report.get("timestamp_utc"),"model":report.get("model"),"backend":backend.get("id"),"artifact":backend.get("artifact") or backend.get("quant"),"backend_fingerprint":{k:backend.get(k) for k in ("id","commit","version","artifact","quant") if backend.get(k) is not None},"context":report.get("context"),"gpus":report.get("gpus"),"gpu_models":[g.get("name") for g in (report.get("hardware") or {}).get("gpus",[])],"gpu_memory_mib":[g.get("memory_total_mib") for g in (report.get("hardware") or {}).get("gpus",[])],"ram_gib":(report.get("hardware") or {}).get("ram_gib"),"cases":[{k:c.get(k) for k in ("name","concurrency","median_prompt_tokens","median_prefill_tok_s","median_decode_tok_s","median_ttft_s","aggregate_output_tok_s")} for c in report.get("cases",[])]}
    QUEUE.mkdir(parents=True, exist_ok=True); name=(compact.get("timestamp_utc") or str(time.time())).replace(":","-")+".json"
