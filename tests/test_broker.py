...
class BrokerTests(unittest.TestCase):
    def setUp(self):
        ...
        self.proc=subprocess.Popen([sys.executable,str(ROOT/'pushbutton-broker'),'--port',str(self.broker_port),'--registry',str(state/'instances.json'),'--no-scaler'],env=env,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
    ...
    def test_context_over_limit_is_rejected_before_backend(self):
        # ~400 prompt tokens + output budget exceeds the 128-token instance limit.
